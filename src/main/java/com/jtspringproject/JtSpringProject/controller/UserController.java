...
public class UserController {
	...
	public ModelAndView indexPage() {
		...
		mv.addObject("products", this.productService.getProducts());
...
//    }
	...
	@Transactional // Atomic: If stock update fails, order won't save and cart won't clear
	public String placeOrder(@RequestParam String address, 
	    ...
	    // 1. Calculate Total & Update Inventory
	    int grandTotal = 0;
	    for (CartProduct item : items) {
	        Product product = item.getProduct();
	        int orderedQty = item.getQuantity();

	        // 🛡️ Safety Check: Ensure enough stock exists
	    ...
	    // 3. Clear Cart only after successful stock update
